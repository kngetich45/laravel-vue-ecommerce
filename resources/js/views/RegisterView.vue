    ...
    registerUser(){
      ...
      Api.post('register', payload)
          .then(response => {
            ...
            console.log(result);
            if (! result.error) {
              this.flash('success', 'Registration Completed', result.message);
              setTimeout(()=>{
                router.push('login');
              }, 2000);
            } else {
              this.flash('error', 'Registration Error', result.message);
            }
          }).catch(error => {
        if (!error.response) {
          // network error
        ...
        this.flash('error', 'Registration Error', errorStatus);
      });
